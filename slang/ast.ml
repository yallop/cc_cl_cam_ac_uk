...
let rand depth =
  ...
  let rec rand_int depth =
    if depth = 0 then
      ...
      | 3 -> BinaryOp (rand_int (depth - 1), Mul, rand_int (depth - 1))
      | 4 -> BinaryOp (rand_int (depth - 1), Div, rand_int (depth - 1))
      | 5 ->
          If (rand_bool (depth - 1), rand_int (depth - 1), rand_int (depth - 1))
      | 6 -> Pair (rand_int (depth - 1), rand_int (depth - 1))
      | 7 -> Fst (rand_int (depth - 1))
      | 8 -> Snd (rand_int (depth - 1))
      | 9 -> Inl (rand_int (depth - 1))
      | _ -> Inr (rand_int (depth - 1))
  and rand_bool depth =
    if depth = 0 then
      Boolean (Random.bool ())
    else
      ...
      | 3 -> BinaryOp (rand_bool (depth - 1), And, rand_bool (depth - 1))
      | 4 -> BinaryOp (rand_bool (depth - 1), Eqb, rand_bool (depth - 1))
      | 5 -> BinaryOp (rand_int (depth - 1), Eqi, rand_int (depth - 1))
      | _ -> BinaryOp (rand_int (depth - 1), Lt, rand_int (depth - 1))
  in
  rand_int depth

let () =
  let _ = rand 6 in
  (* Format.printf "%a@." pp_nice ast; *)
  ()
